...
public class SettingsUtil {
	...
	public static void loadProps(InputStream stream, ROUGESettings rs){
		...
		try {
			...
			String val=properties.getProperty("rouge.metric","N");
			rs.ROUGE_METRIC=val;

			val=properties.getProperty("ngram","1");
			rs.NGRAM=Integer.parseInt(val);

			val=properties.getProperty("alpha","0.8");
			rs.ALPHA=Double.parseDouble(val);
			...
			val=properties.getProperty("embedding","");
			rs.EMBEDDING=val;
